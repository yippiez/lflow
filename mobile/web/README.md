...
- The Go binary embeds the built app (`mobile/dist`), so a plain
  `go build` needs no npm toolchain — dist is committed as a build artifact.

## Server

The daemon is the server (see `tui/daemon/http.go`):
...
`tui/editor/registry.go`: builtins register at startup, and **custom extensions** are ES
