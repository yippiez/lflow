...
// No keyboard sign converts a node's type: "$" is cmd-chip territory — "$$"
// commits the chip even when the command is still EMPTY (a blank $ chip to fill
// in); bash and every other type are set via /type only.
