...
// Host returns the host part of a URL ("https://example.com/x" → "example.com"),
// for a compact default link name. Falls back to the input when unparseable.
func Host(s string) string {
	if u, err := url.Parse(Normalize(s)); err == nil && u.Host != "" {
		return u.Host
	}
