// Workflowy: a thin client for the official Workflowy REST API
// (https://workflowy.com/api-reference/) plus the translate layer that turns
// Workflowy nodes into lflow node fields. The editor's wf node type pulls a
// Workflowy subtree through this package; pushing edits back is a future
// step, which is why the client already carries the write verbs' shapes
...
// WorkflowyClient talks to the Workflowy API. Zero-value plus APIKey is
// usable; BaseURL and HTTP exist so tests inject a mock server.
...
func (c *WorkflowyClient) base() string {
	if c.BaseURL != "" {
...
func (c *WorkflowyClient) http() *http.Client {
	if c.HTTP != nil {
		return c.HTTP
	}
...
func (c *WorkflowyClient) get(ctx context.Context, path string, into interface{}) error {
...
func (c *WorkflowyClient) getNode(ctx context.Context, id string) (Node, error) {
	var n Node
...
func (c *WorkflowyClient) listChildren(ctx context.Context, parentID string) ([]Node, error) {
	var out struct {
...
// walk short.
func (c *WorkflowyClient) FetchSubtree(ctx context.Context, rootID string) (root *TreeNode, truncated bool, err error) {
...
// ParseWorkflowyRef extracts the Workflowy node id from a pasted URL or bare
// id; ok is false when the text holds nothing id-shaped.
func ParseWorkflowyRef(text string) (id string, ok bool) {
