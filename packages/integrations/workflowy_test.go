...
func testClient(srv *httptest.Server) *WorkflowyClient {
	return &WorkflowyClient{APIKey: "test-key", BaseURL: srv.URL, HTTP: srv.Client()}
}
...
func TestFetchSubtreeAuthError(t *testing.T) {
	...
	c := &WorkflowyClient{APIKey: "wrong", BaseURL: srv.URL, HTTP: srv.Client()}
...
func TestParseWorkflowyRef(t *testing.T) {
	...
	for in, want := range cases {
		got, ok := ParseWorkflowyRef(in)
		if !ok || got != want {
			t.Errorf("ParseWorkflowyRef(%q) = %q,%v want %q", in, got, ok, want)
		}
	}
	for _, bad := range []string{"", "just words", "#tag only"} {
		if _, ok := ParseWorkflowyRef(bad); ok {
			t.Errorf("ParseWorkflowyRef(%q) must not match", bad)
		}
	}
}
