...
A client for the lflow daemon: React + TypeScript + Vite.
...
- The Go binary embeds the built app (`packages/mobile/dist`), so a plain
  `go build` needs no npm toolchain — dist is committed as a build artifact.

## Server

The daemon is the server (see `packages/daemon/http.go`):
...
`packages/editor/registry.go`: builtins register at startup, and **custom extensions** are ES
