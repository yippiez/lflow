// Package arithmetic is a tiny, dependency-free expression calculator.
// It parses and evaluates arithmetic expressions like
// "sqrt(x^2 + y^2) + 2*pi" without any external dependencies.
//
...
func envFromJSON(s string) (Env, error) {
	...
	if err := json.Unmarshal([]byte(s), &m); err != nil {
		return nil, fmt.Errorf("arithmetic: invalid variables JSON: %w", err)
	}
	return Env(m), nil
}
