...
// mathMatchTok reports the rune length of the first token in toks (sorted
// longest-first) found at rs[i], 0 when none matches.
func mathMatchTok(toks []string, rs []rune, i int) int {
...
func matchSymOp(rs []rune, i int) (latex string, n int, ok bool) {
	if n := mathMatchTok(mathSymOpTokens, rs, i); n > 0 {
...
func mathBodyTail(n Ref) string {
...
func runMathLatex(h Host, n Ref) tea.Cmd {
	l := mathToLatex(n)
	RunOut(h, n.UUID(), []string{l})
	h.NodeFlash("LaTeX → output")
	return nil
}

// mathFlashActions names the alt+r action "latex" in the flash bar.
func mathFlashActions(h Host, n Ref) []FlashAction {
	return []FlashAction{{Verb: "latex", Color: Theme().Green, Do: runMathLatex}}
}
...
// the same routine feeds the inline preview and structured context. Runs on a
// structure-only ref: Text() falls back to the raw name with no host.
func mathPreview(n Ref) string {
	if n == nil {
		return ""
	}
	...
	kids := make([]string, len(kidRefs))
	for i, c := range kidRefs {
...
// broad, common subset of LaTeX — not its diagram/3-D/tikz corners. Runs on a
// structure-only ref, like mathPreview.
func mathToLatex(n Ref) string {
	if n == nil {
		return ""
	}
	name := strings.TrimSpace(n.Text())
