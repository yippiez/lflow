...
// lm28 adds the artifacts table — runtime-loaded node-type plugins, one JS
...
// lm29 seeds the "log" artifact — the one built-in node type that moved to the
...
// lm30 adds the agent_sessions table — one row per @mention thread, binding a
