...
func (ctx *qCtx) semanticModel() *semanticModel {
	...
	ctx.sem = buildSemanticModel(ctx.cands)
...
// phrase describing it land near each other even with no word in common.
func buildSemanticModel(cands []qCand) *semanticModel {
	m := &semanticModel{df: map[string]int{},
	...
	// pass 1 — a word IS the nodes it appears in
	for _, d := range m.docs {
		idx := randomIndexVector(d.uuid)
		for t, f := range d.tf {
			v := m.termVec[t]
			if v == nil {
...
func randomIndexVector(uuid string) []indexEntry {
	...
	for len(out) < semanticSeeds {
		...
		out = append(out, indexEntry{dim: dim, sign: sign})
	}
	return out
}
