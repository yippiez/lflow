...
// inline-editable type now — including bash/code/query/quote where ">" is real
...
func TestPathChipTriggerEveryInlineType(t *testing.T) {
	for _, typ := range []string{
		database.TypeBullets, database.TypeBash, database.TypeCode,
	...
	for _, typ := range []string{database.TypeBash, database.TypeCode, database.TypeQuery, database.TypeQuote} {
...
func TestAngleCancelTypesLiteral(t *testing.T) {
	m, _ := dbModel(t, database.Node{UUID: "edit", Name: "echo hi ", Type: database.TypeBash})
	cursorOn(m, "edit")
...
func TestAngleKeyTypesLiteralWithoutFzf(t *testing.T) {
	...
	m, _ := dbModel(t, database.Node{UUID: "edit", Name: "echo ", Type: database.TypeBash})
	cursorOn(m, "edit")
...
func TestPathChipInBashNode(t *testing.T) {
	m, _ := dbModel(t, database.Node{UUID: "edit", Name: "cat ", Type: database.TypeBash})
	cursorOn(m, "edit")
