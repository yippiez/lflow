...
Bullets and tree connectors are muted gray and text keeps its own color; the
selected row is marked by its bullet turning red, and the block cursor inverts
the character beneath it. Glyphs: `○` bullet, `●` collapsed, `□` todo / `■`
done, `◆` mirror, and headings show their level digit `1` `2` `3` instead of a
circle.
