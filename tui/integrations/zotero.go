// Zotero: the editor's zotero chip carries one citable reference inline, and
...
// snapshot copy (see zotero_library.go), so a running Zotero is never
// disturbed and the outline never depends on Zotero's schema staying still.
