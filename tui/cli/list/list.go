...
func newRun(ctx runtime.Ctx, opts *options) infra.RunEFunc {
	return func(cmd *cobra.Command, args []string) error {
		...
		switch opts.format {
		case "md":
			out, err = database.RenderMarkdown(db, r.Node, opts.depth, true)
		case "text":
			out, err = database.RenderText(db, r.Node, opts.depth, true)
		case "json":
			out, err = database.RenderJSON(db, r.Node, opts.depth, true)
		default:
