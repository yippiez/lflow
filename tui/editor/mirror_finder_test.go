...
func mirrorTestModel(t *testing.T) (*Model, *database.DB) {
	...
	m := &Model{db: db, ctx: runtime.Ctx{DB: db}, tree: tr, viewStack: []*item{tr.root}, width: 100, height: 30}
	m.refreshRows()
	return m, db
}
...
func zoneModel(t *testing.T) (*Model, *database.DB) {
	...
	m := &Model{db: db, ctx: runtime.Ctx{DB: db}, tree: tr, viewStack: []*item{tr.root}, width: 80, height: 24}
	m.refreshRows()
	return m, db
}
