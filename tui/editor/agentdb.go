...
// trace renders a session's parts as the panel's fixed view. Like the record
// files, a session short enough is shown whole and one that runs past the cap
// shows its most recent bytes — the same head-or-tail rule agentReadTail
// applies, so a long conversation is never paged in whole.
func (db *agentDB) trace(id string) []agentTrace {
	var total int64
	if err := db.db.QueryRow(
		"SELECT COALESCE(SUM(length(data)), 0) FROM part WHERE session_id = ?", id,
	).Scan(&total); err != nil {
		return nil
	}
	skip := total - agentTraceCap
	rows, err := db.db.Query(`SELECT p.data, m.data FROM part p
		JOIN message m ON p.message_id = m.id
		WHERE p.session_id = ? ORDER BY p.time_created ASC`, id)
	if err != nil {
		return nil
	}
	defer rows.Close()
	...
	for rows.Next() {
		var pd, md string
		if err := rows.Scan(&pd, &md); err != nil {
			break
		}
		if skip > 0 {
			pending += int64(len(pd))
			if pending <= skip {
				continue
			}
		}
		var part map[string]any
		if json.Unmarshal([]byte(pd), &part) != nil {
			continue
		}
		role := ""
		var msg map[string]any
		if json.Unmarshal([]byte(md), &msg) == nil {
			role = strings.ToLower(agentString(msg, "role"))
		}
		out = append(out, agentPartTrace(part, role)...)
	}
	return out
}

// agentPartTrace turns one opencode v1 part record into trace rows. The part
// carries the kind of content; the message it belongs to carries the role,
// which is what gives a text part its user or assistant face.
func agentPartTrace(part map[string]any, role string) []agentTrace {
	switch strings.ToLower(agentString(part, "type")) {
	case "text":
		text := strings.TrimSpace(agentString(part, "text"))
		if text == "" {
			return nil
		}
		kind := "user"
		...
		return []agentTrace{{kind: kind, text: oneLine(text)}}
	case "reasoning":
		if text := oneLine(agentString(part, "text")); text != "" {
			return []agentTrace{{kind: "thinking", text: text}}
		}
	case "tool":
		name := agentString(part, "tool")
		var out []agentTrace
		if state, ok := part["state"].(map[string]any); ok {
			// a v1 tool part carries BOTH its call and its result — the input
			// on the tool line, the output as a result, the way the record
			// formats split them into two parts
			if in, ok := state["input"].(map[string]any); ok {
				if text := strings.TrimSpace(name + " " + compactAgentValue(in)); text != "" {
					out = append(out, agentTrace{kind: "tool", text: text})
				}
			}
			if outText, ok := state["output"].(string); ok && strings.TrimSpace(outText) != "" {
				out = append(out, agentTrace{kind: "result", text: clipStr(oneLine(outText), 400)})
			}
		}
		if len(out) == 0 && name != "" {
			out = append(out, agentTrace{kind: "tool", text: name})
		}
		return out
	}
	return nil
